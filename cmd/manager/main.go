...
func main() {
	...
	// Add flags for setting the metrics config
	pflag.String("metrics-host", "0.0.0.0", "operator metrics host")
	pflag.Int32("operator-metrics-port", 8686, "operator metrics port")
	...
	err = leader.Become(ctx, "harbor-operator-lock")
	if err != nil {
		log.Error(err, "")
		os.Exit(1)
	}
	...
	addMetrics(ctx, cfg, namespace,
		...
		viper.GetInt32("metrics-port"),
		viper.GetInt32("operator-metrics-port"),
	)
